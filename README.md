...
4. Obtención del número que representan cada una de las cartas (1-10, J, Q, K)
...
El código implementado para la calibración se encuentra en [src/utils/static/calibracion_camara.py](https://github.com/javier-lazaro/vision-musci/blob/main/src/utils/static/calibracion_camara.py) y el resultado de misma está guardado en [static/npz/calibration_data.npz](https://github.com/javier-lazaro/vision-musci/blob/main/static/npz/calibration_data.npz).
...
2. Aplicación de filtro Gaussiano para reducir la detección de contornos pequeños
3. Obtención de contornos frente al umbral y reordenado de la salida
4. Cálculo del área media y desviación estándar de la muestra de todos los contornos
5. Descarte de contornos que muestren valores atípicos en la distribución --> Contornos muy grandes o muy pequeños
...
<p align="center">
    <img src="https://github.com/user-attachments/assets/7c876051-e7c3-4ff4-9e66-9ebaf6eed9c9" alt="Transformation" width="350"/>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;   
    <img src="https://github.com/user-attachments/assets/c62210ca-842e-469c-9060-63e421966c8e" alt="Transformation v2" width="350"/>
</p>

El resultado es una versión "enderezada" de cada una de las cartas detectadas en la sección inicial. A cada una de estas cartas corregidas se les han aplicado las siguientes operaciones para extraer su color:
...
El uso del umbral para el color amarillo permite que este módulo también sea capaz de determinar si una carta es una carta común (carta entre el 1 y 10) o una figura (J, Q, K), ya que estas últimas son las únicas que tienen un porcentaje significativo de color amarillo. 
...
1. Creación de un rectángulo para analizar región central de interés usando el centroide de cada carta detectada como punto central
...
3. Recuento de contornos obtenidos con lógica adicional para detectar contornos inusualmente pequeños y obviarlos para el conteo
...
Utilizando el modelo YOLO entrenado, el proceso de extracción del palo se realiza de la siguiente manera:

1. Carga del modelo YOLO utilizando los pesos tras el entrenamiento
2. Obtención de las clases predichas para los píxeles transformados tras la extracción del color
...
4. Operación de post procesado adicional para comparar la clase detectada (palo) con el color extraído de la carta.
   ...
   - Si la predicción es imposible dado el color, se aplica una corrección manual en base al vecino más próximo. Se devuelve el palo corregido.
